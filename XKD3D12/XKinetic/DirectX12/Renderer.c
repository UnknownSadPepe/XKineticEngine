...
XkResult xkD3D12CreateRenderer(XkD3D12Renderer* pRenderer, XkRendererConfig* const pConfig, XkWindow window) {
	xkAssert(pRenderer);
	xkAssert(pConfig);
	xkAssert(window);
	...
	*pRenderer = xkAllocateMemory(sizeof(struct XkD3D12Renderer));
	if(!pRenderer) {
...
void xkD3D12ClearColorRenderer(XkD3D12Renderer renderer, XkVec4 color) {
	xkAssert(renderer);
...
void xkD3D12ClearDepthRenderer(XkD3D12Renderer renderer, XkFloat32 depth) {
	xkAssert(renderer);
	/// TODO: Implementation.
}

void xkD3D12ClearStencilRenderer(XkD3D12Renderer renderer, XkUInt32 stencil) {
	xkAssert(renderer);
	/// TODO: Implementation.
}
...
void xkD3D12TopologyRenderer(XkD3D12Renderer renderer, XkTopology topology) {
	xkAssert(renderer);
	/// TODO: Implementation.
}

void xkD3D12CullModeRenderer(XkD3D12Renderer renderer, XkCullMode cullMode) {
	xkAssert(renderer);
	/// TODO: Implementation.
}
...
void xkD3D12ResizeRenderer(XkD3D12Renderer renderer, XkSize width, XkSize height) {
	xkAssert(renderer);
...
void xkD3D12ScissorRenderer(XkD3D12Renderer renderer, XkInt32 x, XkInt32 y, XkSize width, XkSize height) {
	xkAssert(renderer);
...
void xkD3D12Draw(XkD3D12Renderer renderer, XkSize vertexCount) {
	xkAssert(renderer);
...
void xkD3D12DrawIndexed(XkD3D12Renderer renderer, XkSize indexCount) {
	xkAssert(renderer);
...
XkResult xkD3D12CreateBuffer(XkD3D12Buffer* pBuffer, const XkBufferUsage usage, const XkSize size, XkHandle data, XkD3D12Renderer renderer) {
	xkAssert(pBuffer);
	xkAssert(renderer);

	XkResult result = XK_SUCCESS;

	*pBuffer = (XkD3D12Buffer)xkAllocateMemory(sizeof(struct XkD3D12Buffer));
	if(!pBuffer) {
		result = XK_ERROR_BAD_ALLOCATE;
		goto _catch;
	}
...
XkResult xkD3D12CreateTexture2d(XkD3D12Texture2d* pTexture, XkHandle data, const XkSize width, const XkSize height, XkD3D12Renderer renderer) {
	xkAssert(pTexture);
	xkAssert(renderer);

	XkResult result = XK_SUCCESS;

	*pTexture = (XkD3D12Texture2D)xkAllocateMemory(sizeof(struct XkD3D12Texture2D));
	if(!pTexture) {
		result = XK_ERROR_BAD_ALLOCATE;
		goto _catch;
	}
